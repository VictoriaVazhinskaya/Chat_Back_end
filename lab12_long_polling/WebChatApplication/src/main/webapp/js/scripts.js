...
function longPolling(){
	ajax('GET', appState.mainUrl + '?token=' + formToken(numberOfChanging), null, function(responseText){
		...
		setServerAvailable();
		console.error("abcdefg");
        longPolling();
	}, function(error) {
		defaultErrorHandler(error);
		console.error("in do polling");
		//setTimeout(loop, 500);
	});
}
